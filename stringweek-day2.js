// take in string
// return string with everything up to 'c' removed >> remove all instances of 'Eek!' >> reverse string >> mystery step (split on 'v') >> note first character on each array item

function unscrambleThisMess (str) {
    // slice string at index of 'c' + 1
    let uncatScratchedStr = str.slice(str.indexOf('c')+1)
    // remove all 'Eek!'s via split
    let eeklessStr = uncatScratchedStr.split('Eek!').join('')
    // reverse str
    let reversedStr = eeklessStr.split('').reverse().join('')
    // find mystery step (split on 'v')
    let splitStr = reversedStr.split('v')
    let secret = ''
